...
echo "[build_vllm] pip install -e third_party/vllm --no-build-isolation"

cd "$ROOT/third_party/vllm"
pip install -e . --no-build-isolation

# vLLM 0.10.2 声明 transformers>=4.55.2 但过松；transformers 5.x 有破坏性改动
# (ProcessorMixin)，需钉在 4.x。同时补齐 XiRang 自身依赖。
pip install -q "transformers>=4.55.2,<5.0" pynvml matplotlib pytest
