...
- Node ≥ 22.19.0（本机用 nvm 装的 v24.18.0：`nvm install 24 && nvm use 24`）
- pnpm 11.2.2（`corepack enable && corepack prepare pnpm@11.2.2 --activate`）
- 安装依赖：`cd third_party/openclaw && pnpm install --frozen-lockfile`
  - ⚠️ 本机系统 cmake 3.16 < 3.19，`extensions/llama-cpp` 的 `node-llama-cpp` postinstall 会编译失败。
    我们走 vLLM 不需要本地 llama 推理，跳过它：`NODE_LLAMA_CPP_SKIP_DOWNLOAD=true pnpm install --frozen-lockfile`
- 构建：`pnpm build`（约 4-5 分钟，产出 dist + CLI 启动元数据；`config patch`/`infer` 等命令依赖构建产物）
- 设环境变量：`export VLLM_API_KEY=vllm-local`（本地无鉴权，任意非空值即可）
- 应用配置：`openclaw config patch --file third_party/openclaw/xirang.patch.json5`
